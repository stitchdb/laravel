...
class StitchDBConnection extends Connection
{
    ...
    public function __construct($pdo, $database = '', $tablePrefix = '', array $config = [])
    {
        ...
        // Set a reconnector so Laravel doesn't throw LostConnectionException
        $this->setReconnector(function ($connection) {
            // HTTP is stateless — nothing to reconnect, just continue
        });
    }
    ...
    public function select($query, $bindings = [], $useReadPdo = true)
    {
        return $this->run($query, $bindings, function ($query, $bindings) {
            // Convert :named params to ? positional params for the API
    ...
    {
        return $this->unprepared('PRAGMA foreign_keys = ON');
    }

    /**
     * Disable foreign key constraints (PRAGMA foreign_keys = OFF).
     *
     * @return bool
     */
    public function disableForeignKeyConstraints()
    {
        return $this->unprepared('PRAGMA foreign_keys = OFF');
    }

    /**
     * Run a PRAGMA command and return its result.
     *
     * @param string $name The pragma name (e.g. 'foreign_keys', 'table_info("users")')
     * @return array
     */
     ...
     *
     * Takes a SQL string with :named placeholders and an associative array of
     * bindings, and returns [sql, positional_bindings] where the SQL uses ?
     * placeholders and the bindings are in the correct positional order.
     *
     * @param string $sql
    ...
    {
        // If bindings are already numerically indexed, nothing to convert
        if (empty($bindings)) {
            return [$sql, $bindings];
        }
        ...
        $positional = [];
        $converted = preg_replace_callback('/:([a-zA-Z_][a-zA-Z0-9_]*)/', function ($matches) use ($bindings, &$positional) {
            $name = $matches[1];
            $prefixed = ':' . $name;

            if (array_key_exists($prefixed, $bindings)) {
                $positional[] = $bindings[$prefixed];
            } elseif (array_key_exists($name, $bindings)) {
                $positional[] = $bindings[$name];
            } else {
                $positional[] = null;
            }
